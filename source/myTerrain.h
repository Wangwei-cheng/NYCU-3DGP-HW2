...
//#include "./others_samples/sdksample.h"
...
//#define TERRAIN_SHOW_MSG


class  MyTerrain
{
public:
    MyTerrain(SceneManager* sceneMgr, Camera *camera, SceneNode *cameraNode, TrayManager *trayMgr)
    ...
    public:
        ...
        Vector3 getTerrainSurfacePoint(const Vector3& p, double offset_y = 1.0) {
            Vector3 q = p;
            Ray ray;
            ray.setOrigin(Vector3(p.x, mTerrainPos.y + 10000, p.z));
            ray.setDirection(Vector3::NEGATIVE_UNIT_Y);

            TerrainGroup::RayResult rayResult = mTerrainGroup->rayIntersects(ray);
            if (rayResult.hit)
            {
                q.y = rayResult.position.y + offset_y;
            }
            return q;
        }
    bool frameRenderingQueued(const FrameEvent& evt) 
    {
        if ( true || mMode != MODE_NORMAL)
        {
...
#ifdef TERRAIN_SHOW_MSG
            std::cout << "Terrain::mCamera:" << mCamera << std::endl;
            std::cout << "Terrain::mTrayMgr:" << mTrayMgr << std::endl;


            std::cout << "rpos:" << p.x << std::endl;
            std::cout << "rpos:" << p.y << std::endl;
            std::cout << "rpos:" << p.z << std::endl;
#endif
            ...
            if (rayResult.hit)
            {
                ...
                for (TerrainGroup::TerrainList::iterator ti = terrainList.begin(); ti != terrainList.end(); ++ti) {
                    ...
                    std::cout << "asd" << *ti << std::endl;
                    std::cout << "evt.timeSinceLastFrame:" << evt.timeSinceLastFrame << std::endl;
                    std::cout << "pos:" << rayResult.position.x << std::endl;
                    std::cout << "pos:" << rayResult.position.y << std::endl;
                    std::cout << "pos:" << rayResult.position.z << std::endl;
...
//            std::cout << "mFly:" << mFly << std::endl;
    ...
    {
            std::cout << "itemSelected" << std::endl;
        if (menu == mEditMenu)
        {
        ...
        {
            std::cout << "mShadowsMenu" << std::endl;
...
protected:
    ...
    public:
        virtual Vector3 getMarkerPosition() const {
            return mEditNode->getPosition();
        }
    void setupContent()
    {
        //! [global_opts]
     ...
     //   setDragLook(true);
        ...
        l->setDiffuseColour(ColourValue(0.3, 0.3, 0.3));
        ...
        ln->setDirection(Vector3(0.55, -10, 0.75).normalisedCopy());
        ln->attachObject(l);
        //! [light]
